  ...
  template: `
    <div class='space-y-6'>
      <div class='flex justify-between items-center'>
        <div>
          <h2 class='text-2xl font-black text-gray-900'>Deployments</h2>
          <p class='text-sm text-gray-500'>Manage your deployments</p>
        </div>
        <a routerLink='/deployments/new' class='koda-btn-primary'>Add Deployment</a>
      </div>
      <div class='koda-card overflow-hidden'>
        <table class='w-full text-left'>
          <thead>
            <tr class='bg-gray-50'>
              <th class='px-6 py-4 text-xs font-bold text-gray-500 uppercase'>version</th><th class='px-6 py-4 text-xs font-bold text-gray-500 uppercase'>deployed at</th><th class='px-6 py-4 text-xs font-bold text-gray-500 uppercase'>application id</th>
          ...
          <tbody class='divide-y divide-gray-100'>
            <tr *ngFor='let item of items()' class='hover:bg-gray-50 transition-colors'>
              <td class='px-6 py-4 font-bold text-gray-700'>{{ item.version }}</td><td class='px-6 py-4 font-bold text-gray-700'>{{ item.deployed_at | date:'medium' }}</td><td class='px-6 py-4 font-bold text-gray-700'>{{ item.application_id }}</td>
              <td class='px-6 py-4 text-right'>
                <div class='flex items-center justify-end gap-2'>
                  <a [routerLink]="['/deployments/edit', item.id]" class='text-xs font-bold text-blue-600 border border-blue-200 px-3 py-1.5 rounded-lg'>Edit</a>
            ...
            <tr *ngIf='items().length === 0'>
              <td colspan='4' class='px-6 py-12 text-center text-gray-400 italic'>No data found.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div *ngIf='showDeleteModal' class='fixed inset-0 z-50 flex items-center justify-center p-4'>
      ...
      <div class='relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-0 overflow-hidden'>
        <div class='p-8 text-center'>
          <h3 class='text-xl font-black text-gray-900 mb-2'>Delete Deployment</h3>
