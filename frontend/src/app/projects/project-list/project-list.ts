  ...
  template: `
    <div class='space-y-6'>
      <div class='flex justify-between items-center'>
        ...
        <a routerLink='/projects/new' class='koda-btn-primary'>Add Project</a>
      </div>
      <div class='koda-card overflow-hidden'>
        <table class='w-full text-left'>
          <thead>
            <tr class='bg-gray-50'>
              <th class='px-6 py-4 text-xs font-bold text-gray-500 uppercase'>name</th><th class='px-6 py-4 text-xs font-bold text-gray-500 uppercase'>description</th><th class='px-6 py-4 text-xs font-bold text-gray-500 uppercase'>start date</th><th class='px-6 py-4 text-xs font-bold text-gray-500 uppercase'>end date</th>
          ...
          <tbody class='divide-y divide-gray-100'>
            <tr *ngFor='let item of items()' class='hover:bg-gray-50 transition-colors'>
              <td class='px-6 py-4 font-bold text-gray-700'>{{ item.name }}</td><td class='px-6 py-4 font-bold text-gray-700'>{{ item.description }}</td><td class='px-6 py-4 font-bold text-gray-700'>{{ item.start_date | date:'mediumDate' }}</td><td class='px-6 py-4 font-bold text-gray-700'>{{ item.end_date | date:'mediumDate' }}</td>
              <td class='px-6 py-4 text-right'>
                <div class='flex items-center justify-end gap-2'>
                  <a [routerLink]="['/projects/edit', item.id]" class='text-xs font-bold text-blue-600 border border-blue-200 px-3 py-1.5 rounded-lg'>Edit</a>
    ...
    <div *ngIf='showDeleteModal' class='fixed inset-0 z-50 flex items-center justify-center p-4'>
      ...
      <div class='relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-0 overflow-hidden'>
        <div class='p-8 text-center'>
          <h3 class='text-xl font-black text-gray-900 mb-2'>Delete Project</h3>
